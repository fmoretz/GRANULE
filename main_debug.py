...
for index in range(0, len(day_sim)):
	...
	axs[0,1].plot(t, _S, 'b--', linewidth=0.4, label='S')
	axs[0,1].plot(t_ss, S_ss, 'b-o', linewidth=1)
	axs[0,1].set_xlabel('t - d')
	axs[0,1].set_ylabel('S, - kg/m3')
	axs[0,1].set_xlim([0, t[-1]+10])
	axs[0,1].set_title('Substrate Concentration Profile', fontsize=10)
	axs[0,1].grid(True)

	axs[1,0].plot(t, _X, 'y--', linewidth=0.4, label='X')
	axs[1,0].plot(t_ss, X_ss, 'y-o', linewidth=1)
	axs[1,0].set_xlabel('t - d')
	axs[1,0].set_ylabel('X - kg/m3')
	axs[1,0].set_xlim([0, t[-1]+10])
	axs[1,0].set_ylim([0, 1.5*Xo])
	axs[1,0].set_title('Active Biomass Concentration Profile', fontsize=10)
	axs[1,0].grid(True)


	axs[1,1].plot(t, _E, 'm--', linewidth=0.4, label='E')
	axs[1,1].plot(t_ss, E_ss, 'm-o', linewidth=1)
	...
	axs2[1,1].plot(np.linspace(1, N, N-2), _Vbed[1:-1], 'b-o', linewidth=2)
