...
for index in range(0, len(day_sim)):
	...
	# ∞∞ VISZ ∞∞
	t = np.linspace(0, 50, len(_S))
	t_ss = np.linspace(0, 50, len(M_ss))
	...
	axs[1,0].plot(t_ss, S_ss_graph, 'b-', linewidth=1)
	axs[1,0].plot([0, 10, 20, 30, 40, 50], [5.5, 4.2, 2.7, 1.5, 0.2, 0], 'rs', linewidth=0.7)
	axs[1,0].plot([0, 10, 20, 30, 40, 50], [5.5, 4.3, 2.9, 1.4, 0.2, 0], 'yo', linewidth=0.7)
	...
	axs[3,0].set_xlabel('t - h')
	axs[3,0].set_ylabel('M - kg/m3')
	axs[3,0].set_xlim([0, t[-1]])
	...
	axs[3,1].set_xlabel('t - h')
	axs[3,1].set_ylabel('M - m3/d')
	axs[3,1].set_xlim([0, t[-1]])
