...
for index in range(0, len(day_sim)):
	...
	for k in range(1, N+1):
		...
		if (Xo + Eo) >= 0.25*rho_biomass:
			...
			[So, Xo, Eo, To, Mo],
			method = 'RK23',
			args=(Q, Vef/N, So, _R, Y, Kd, eta, U, A_lateral, rho_biomass, cp, m, To, Tw, Na, Ks, _m, Xo, wi),
			atol=1e-7,
			rtol=1e-9,
			#mxstep=100000
			)
			...
			#Simulation plot
			tspan = np.linspace(0, 20, len(S))
			plt.plot(tspan, S*Xinitial, label="n="+str(k))
			plt.grid(True)
			plt.xlabel('Time [h]')
			plt.ylabel('Substrate Concentration [kg/m3]')
			plt.legend(loc='best')
