...
from physical_coefficients_np import *
...
for index in range(0, len(day_sim)):
	...
	Ar  = (rho_H2O * gravity * (rho_biomass - rho_H2O) * Dp**3) / (viscosity_H2O/1000)**2
	bed_porosity  = (( (18*Re + 2.7*Re**(1.69)) /Ar)**(1/4.7))

	Np = round(Vbed *(1-bed_porosity)/ (4/3 * 3.14 * _Rnew[-1]**3))
	...
	for k in range(1, N+1):
		...
		[So, Xo, Eo, To, Mo],
		t,
		args=(Q, Vef/N, So, Xo, Eo, _R, Y, Kd, eta, U, A_lateral, rho_biomass, cp, m, To, Tw, w, _Rnew[-1], D, rho_biomass),
		atol=1e-7,
		rtol=1e-9,
		mxstep=100000
		)
		...
		Rnew = (3/4 * Vbed*(1-bed_porosity)/3.1415/Np)**(1/3)
		print(f'updated radius: {Rnew} m')
		...
		Ar  = (rho_H2O * gravity * (rho_biomass - rho_H2O) * Dp**3) / (viscosity_H2O/1000)**2
		bed_porosity  = (( (18*Re + 2.7*Re**(1.69)) /Ar)**(1/4.7))
		print(f'bed porosity: {bed_porosity} m')
		print(f'At CSTR: {k} ')
		...
		Thiele = _Rnew[-1] / 3 *(_m/Y *Xo /(Ks + Sp[-1])/D )**0.5 
