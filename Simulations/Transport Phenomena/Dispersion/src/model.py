...
def model():
	...
	fig_pathway = '../output/'
	fig.savefig(fig_pathway + 'Concentration profiles.svg')

	fig2, axs2 = plt.subplots(2, 2, figsize=(25, 10))
	plt.subplots_adjust(
            left   = 0.125,
            bottom = 0.071,
            right  = 0.9,
	...
	axs2[0,0].plot(np.linspace(1, N, N), _Pe_TIS, 'k', linewidth=2, marker='.', markersize=10, linestyle='solid')
	axs2[0,0].set_xlabel('N° CSTR')
	axs2[0,0].set_ylabel('Pe - [-]')
	axs2[0,0].grid(color='k', alpha=0.5, linestyle='dashed', linewidth=0.8)
	axs2[0,0].set_xlim([0, N])
	axs2[0,0].set_title('Peclet number - from N° CSTR', y=1.025, fontsize=18)
	axs2[0,1].plot(np.linspace(1, N, N), _Pe, 'k', linewidth=2, marker='.', markersize=10, linestyle='solid')
	axs2[0,1].set_xlabel('N° CSTR')
	axs2[0,1].set_ylabel('Pe - [-]')
	axs2[0,1].set_xlim([0, N])
	axs2[0,1].grid(color='k', alpha=0.5, linestyle='dashed', linewidth=0.8)
	axs2[0,1].set_title('Peclet number - characteristic length: Dp',  y=1.025, fontsize=18)
	axs2[1,0].plot(np.linspace(1, N, N), _Dispersion_coefficient, 'k', linewidth=2, marker='.', linestyle='solid')
	axs2[1,0].set_xlabel('N° CSTR')
	axs2[1,0].set_ylabel('Dispersion coefficient - [m$^2$/h]')
	axs2[1,0].set_xlim([0, N])
	axs2[1,0].grid(color='k', alpha=0.5, linestyle='dashed', linewidth=0.8)
	axs2[1,0].set_title('Dispersion coefficient - experimental relationship',  y=1.025, fontsize=18)
	axs2[1,1].plot(np.linspace(1, N, N), _D_TIS, 'k', linewidth=2, marker='.', linestyle='solid')
	axs2[1,1].set_xlabel('N° CSTR')
	axs2[1,1].set_ylabel('Dispersion coefficient - [m$^2$/h]')
	axs2[1,1].set_xlim([0, N])
	axs2[1,1].grid(color='k', alpha=0.5, linestyle='dashed', linewidth=0.8)
	axs2[1,1].set_title('Dispersion coefficient - from TIS',  y=1.025, fontsize=18)

	fig_pathway = '../output/'
	fig2.savefig(fig_pathway + 'Axial dispersion.svg')
	...
	file_output.write('=======================\n')
	file_output.write(f'RESULTS AFTER {round(day)} DAYS OF SIMULATION:\n')
	file_output.write('=======================\n')
	file_output.write(f'Efficiency of substrate removal:          {round(removal_efficiency, 2)} %\n')
	file_output.write(f'Final Substrate concentration:            {round(_S_graph[-1], 2)} [kg/m3]\n')
	file_output.write(f'Final Biomass concentration:  	          {round(_X_graph[-1], 2)} [kg/m3]\n')
	file_output.write(f'Final Methane volumetric concentration:   {round(_M_vol_graph[-1], 2)} [m3/d]\n')
	file_output.write(f'Daily average Methane production:         {round(M_production_average, 2)} [m3/d]\n')
	file_output.write(f'Radius particle growth:                   {round(radius_growth, 2)} %\n')
	file_output.write(f'Final particle radius:                    {round(_Rnew[-1], 2)} mm')
	file_output.close()
