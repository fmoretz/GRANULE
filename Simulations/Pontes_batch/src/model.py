...
def model():
	...
	S_ss = np.zeros(N+1); X_ss = np.zeros(N+1); E_ss = np.zeros(N+1); T_ss = np.zeros(N+1)
	S_ss[0] = So; X_ss[0] = Xo; E_ss[0] = Eo; T_ss[0] = To
	wM =0;
	...
	t_temp = np.linspace(	# Time span definition
	0,					# Start - h
	day*24,			    # End   - h
	npt					# Number of iter
	)

	m   = ((Xo + Eo)*Xinitial + rho_H2O) * Vef;  		# kg
	...
	[To],
	t_temp,
	args=(Q, U, A_lateral, rho_biomass, cp, m, To, Tw),
	atol=1e-7,    
	rtol=1e-9,
	mxstep=100000,
	hmax=0.1
	...
	for k in range(1, N+1):
		...
		[So, Xo, Eo, Mo],
		t,
		args=(Q, Vef/N, So, Xo, Eo, _R, Y, Kd, eta, w),
		atol=1e-7,    
		rtol=1e-9,
		mxstep=100000,
		hmax=0.1
	...
	t_exp = [0, 10/24, 20/24, 30/24, 40/24, 50/24]
	S_bolle = [5.5, 4.2, 2.7, 1.5, 0.2, 0.0]
	S_pinto = [5.5, 4.3, 2.9, 1.4, 0.2, 0.0]
	axes.plot(t_ss, S_ss_graph, 'k', linewidth=2.5, linestyle='solid', label='model')
	axes.plot(t_exp, S_bolle, 'r^' ,label='Bolle et al')
	axes.plot(t_exp, S_pinto, 'bs' ,label='Pinto et al')
	axes.set_xlabel('time - [d]')
	axes.set_ylabel('Effluent substrate - [kg/m$^3$]')
	axes.grid(color='k', alpha=0.5, linestyle='dashed', linewidth=0.8)
	axes.set_title('Effluent substrate', y=1.025, fontsize=18)
	axes.legend(loc='best')

	fig_pathway = '../output/Figures/'
	figure.savefig(fig_pathway + 'Substrate_batch.svg')	
	...
	axs[0,1].plot(t, _X_graph, 'k', linewidth=2, marker='.', markersize=10, markevery=round(len(t)/N), linestyle='solid', label='X')
	...
	axs[0,2].plot(t, _E_graph, 'k', linewidth=2, marker='.', markersize=10, markevery=round(len(t)/N), linestyle='solid', label='E')
	...
	t_temp_graph = np.array(t_temp)/24
	axs2.plot(t_temp_graph, (np.array(T)-273.15), 'k', linewidth=2, marker='.', markersize=10, linestyle='solid')
	axs2.set_xlabel('t - [d]')
	axs2.set_ylabel('T - [°C]')
	axs2.grid(color='k', alpha=0.5, linestyle='dashed', linewidth=0.8)
	axs2.set_title('Temperature', y=1.025, fontsize=18)
	axs2.set_xlim([0, t_temp_graph[-1]])


	fig2_pathway = '../output/Figures/'
	fig2.savefig(fig_pathway + 'Temperature'+str(date.tm_mday)+'_'+str(date.tm_mon)+'_'+str(date.tm_year)+'.png')
	...
	sheet4 = wb.add_worksheet('Temperature')
	sheet4.insert_image(1,1, '../output/Figures/Temperature'+str(date.tm_mday)+'_'+str(date.tm_mon)+'_'+str(date.tm_year)+'.png')
